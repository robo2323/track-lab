...
import wav from '../../samples/808_drum_kit/hihats/808-HiHats01.wav';

class ClosedHiHat extends Component {
  constructor(props) {
    super(props);
    this.state = {
      playing: false
    ...
    const notes = new Array(16).fill(null);
     this.closedHiHat = new Tone.Player({
      url: wav
    }).toMaster();
    this.loop = new Tone.Sequence(
      (time, note) => {
        this.closedHiHat.start();
      },
      notes,
      '16n'
    );
  ...
  componentDidUpdate() {
    ...
    if (this.props.volume && +this.props.volume > -15) {
      this.closedHiHat.set('volume', this.props.volume);
    } else if (this.props.volume && +this.props.volume === -15) {
      this.closedHiHat.set('volume', -100);
    }
    if (this.props.notes) {
