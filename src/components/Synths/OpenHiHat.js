...
import wav from '../../samples/808_drum_kit/hihats/808-OpenHiHats06.wav';

class OpenHiHat extends Component {
  constructor(props) {
    super(props);
    this.state = {
      playing: false
    ...
    this.openHiHat = new Tone.Player({
      url: wav
    }).toMaster();
    this.loop = new Tone.Sequence(
      (time, note) => {
        this.openHiHat.start();
      },
      notes,
      '16n'
    );
  ...
  componentDidUpdate() {
    ...
    if (this.props.volume && +this.props.volume > -15) {
      this.openHiHat.set('volume', this.props.volume);
    } else if (this.props.volume && +this.props.volume === -15) {
      this.openHiHat.set('volume', -100);
    }
    if (this.props.notes) {
